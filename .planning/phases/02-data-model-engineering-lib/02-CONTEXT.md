...
- **D-09a (amendment, ruled 2026-08-20 after Phase 2 research inspected real data):** Live inspection of real January 2019 TLC data found ~1% of rows with non-positive `trip_distance`/duration and a small rate of zone IDs 264/265 (`Unknown`/`Outside of NYC`) in every sampled month — under pandera's default per-row `validate()` semantics, D-09's checks as literally read would reject essentially every one of the 12 real months, not just genuinely malformed ones. Two enforcement tiers replace the single "any violation fails the month" reading: **structural checks** (missing/renamed columns, wrong dtypes, zone IDs outside `[1, 263]`, i.e. malformed beyond the known 264/265 case) still cause a loud whole-month rejection — this is what "malformed/schema-drifted month fails loudly" (REQ-C1) actually protects against. **Row-level numeric-quality checks** (non-positive distance/duration, the 264/265 zone-ID case) are pre-filtered and the filtered count is logged before schema validation runs, rather than failing the whole month. This is not silent filtering — REQ-C1's "not silently" is satisfied by the logged count — and it keeps the schema genuinely enforcing structural integrity rather than becoming a no-op that trivially fails every real month it's pointed at. — **Reversibility:** reversible — tightening back to hard-fail-on-any-violation is a one-line config/check change, at the cost of reopening the every-month-fails problem this amendment closes.
