...
- [x] **Phase 2: Data & Model Engineering (lib/)** - 100% of pandas/numpy feature logic, pandera schema validation, vectorized haversine + dtype downcasting + benchmark table, LightGBM training, and an MLflow registry client wrapper — written and unit-tested entirely offline, no cluster required. (completed 2026-08-22)
...
**Success Criteria** (what must be TRUE):
  ...
  3. `pytest` runs against `lib/` asserting exact output values (DataFrame fixtures deferred to Phase 2, where pandas first enters scope per CONTEXT.md's phase boundary); GitHub Actions blocks a PR with a broken `lib/` change at the failing stage, while a clean PR produces a pushed GHCR-tagged component image.
...
- [x] 02-03-PLAN.md — Modelling tail: chronological drift-spanning split (D-08), single fixed untuned LightGBM config (REQ-D2), RMSE evaluation, and the alias-based MLflow registry wrapper against a mocked client (REQ-D3)

**Wave 3** *(blocked on Wave 2)*

- [x] 02-04-PLAN.md — Ingest boundary: chunked reads (REQ-C4), counted-and-logged row-level quality pre-filter and pandera structural schema per D-09/D-09a (REQ-C1), proven against all 12 real months

**Wave 4** *(blocked on Wave 3)*

- [x] 02-05-PLAN.md — Vectorized zone-centroid haversine (REQ-C2), the NaN-safe dtype-downcasting contract (REQ-C3), and the generated README benchmark table plus drift-window rationale (REQ-C5, REQ-D1)
...
- [x] 03-04-PLAN.md — Cluster bring-up under D-10's time-box: k3d, KFP standalone 2.17.0 on the platform-agnostic overlay, dedicated MinIO and in-cluster MLflow (D-12), credentials, raw-data upload, submit and checksum tooling, and the first real per-month branch. **HALTED**: tasks 1-2 and Task 3's tooling are complete and verified. The original GHCR-image-publish blocker is resolved (branch pushed, PR #10 open, CI green). Two real bugs found and fixed via live-cluster testing (adapter-function memory hygiene; a missing Dockerfile COPY), and PARALLELISM cut 2->1 (documented REQ-B5 deviation). The full per-month-branch mechanism is proven end-to-end with GHCR-pulled images against a truncated synthetic month, but the plan's literal full-real-data run (2020-02 through 2020-04) remains unverified pending more host memory headroom - 03-05 needs that full run and cannot substitute the truncated proof. See 03-04-SUMMARY.md Known Gap.

**Wave 4** *(blocked on Wave 3)*

- [ ] 03-05-PLAN.md — Behaviour proofs on the cluster: full DAG to `register` and the documented skip branch, promotion both directions, byte-identical backfill on the D-13 subset, failure-path `ExitHandler` evidence, cache invalidation, and traceability closure
