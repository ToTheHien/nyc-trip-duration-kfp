...
### pre-commit Hook Set
- **D-03:** `pre-commit` config mirrors CI exactly: `ruff` + `mypy --strict` on `lib/`, nothing more. No added hygiene hooks (trailing-whitespace, YAML/TOML validation, etc.) — minimize setup friction for the 10-15h solo budget.
